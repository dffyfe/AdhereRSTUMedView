...
.SVG.rect <- function(x=NA, y=NA, width=NA, height=NA, xend=NA, yend=NA,  # can accomodate both (wdith,height) and (xend,yend)
                      stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, fill="white", fill_opacity=NA, shape_rendering=NA, other_params=NA, # styling attributes
...
)
{
  ...
  r <- list(# The initial comment (if any):
    ...
    if(!is.na(shape_rendering)) c('shape-rendering="',shape_rendering,'" '),
    # Other parameters:
...
#' @export
last.plot.get.info <- function() { return (get(".last.cma.plot.info", envir=.adhererstumedview.env)); }
...
get.legend.plotting.area <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    if( is.null(lcpi) || is.null(lcpi$baseR) )
    {
      if( !suppress.warnings ) .report.ewms("No CMA plot or no base R was generated!\n", "error", "get.legend.plotting.area", "AdhereRSTUMedView");
      return (NULL);
    } else
    {
...
get.plotted.events <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    if( is.null(lcpi) || is.null(lcpi$baseR) )
    {
      if( !suppress.warnings ) .report.ewms("No CMA plot or no base R was generated!\n", "error", "get.plotted.events", "AdhereRSTUMedView");
      return (NULL);
    } else
    {
...
get.plotted.partial.cmas <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    if( is.null(lcpi) || is.null(lcpi$baseR) )
    {
      if( !suppress.warnings ) .report.ewms("No CMA plot or no base R was generated!\n", "error", "get.plotted.partial.cmas", "AdhereRSTUMedView");
      return (NULL);
    } else
    {
...
.plot_summary.CMAs <- function(cma,                                   # the CMA_per_episode or CMA_sliding_window (or derived) object
                       ...
                       lty.event="solid", lwd.event=2, pch.start.event=21, pch.end.event=16, # event style
...
)
{
  ...
  .do.SVG <- (!is.null(export.formats) && any(c("svg", "html", "jpg", "png", "webp", "ps", "pdf") %in% export.formats));
  if( !.do.SVG )
  {
    # Nothing to plot!
  ...
  if( !is.numeric(html.plot.number) || !is.numeric(total.plots.in.html) ) .report.ewms("Both 'html.plot.number' and 'total.plots.in.html' must be numbers!", "error", ".plot.CMAs", "AdhereRSTUMedView", error.as.warning=FALSE);

  # Throw error if number of custom plot names > 10
  if( use.custom.plots==TRUE && length(custom.plot.names)>10 ) .report.ewms("html template can only currently accommodate up to 10 custom plots.", "error", ".plot.CMAs", "AdhereRSTUMedView", error.as.warning=FALSE);

  # Throw error if logo file doesn't exist in specified directory
  if( logo.to.use != 'AdhereR' && !file.exists(paste0(export.formats.directory, '/', logo.to.use))) .report.ewms("Logo file doesn't exist in the specified 'export.formats.directory', please try removing '/' from 'export.formats.directory' or 'logo.to.use' or both!", 'error', ".plot.CMAs", "AdhereRSTUMedView");

  # Throw an error if logo dimensions aren't numbers
  if( logo.to.use != 'AdhereR' && (!is.numeric(custom.logo.width) || !is.numeric(custom.logo.height))) .report.ewms("'custom.logo.width' and 'custom.logo.height' must both be numeric!", "error", ".plot.CMAs", "AdhereRSTUMedView");

  # Throw error is length(custom.plot.names) != total.plots.in.html
  if( use.custom.plots==TRUE && length(custom.plot.names) != total.plots.in.html) .report.ewms("Lenght of custom.plot.names does not equal total.plots.in.html", "error", ".plot.CMAs", "AdhereRSTUMedView", error.as.warning=FALSE);
  ...
  {
    .report.ewms("show.event.intervals must be FALSE on sliding window steps > 1 and on CMA0", "error", ".plot.CMAs", "AdhereRSTUMedView");
  ...
  if( show.legend )
  {
    if( .do.SVG )
    {
      .legend.SVG <- function(x=0, y=0, do.plot=TRUE)
      {
        ...
        {
          l2 <- c(l2,
                  .SVG.points(x=x.origin + lmx, y=y.origin + lmy+lh,
                              pch=pch.start.event, col=bar.cols[1], cex=legend.cex*1.5,
        ...
        {
          l2 <- c(l2,
                  .SVG.text(x=x.origin + lmx + 4*dims.chr.legend, y=y.origin + lmy+lh, text="Mapped by software",
          ...
          .last.cma.plot.info$SVG$legend$components <<- rbind(.last.cma.plot.info$SVG$legend$components,
                                                              data.frame("string"="Mapped by software",
        ...
        lh <- lh + lnl*dims.chr.legend; lw <- max(lw, .SVG.string.dims("Mapped by software", font_size=dims.chr.legend)["width"] + 2*dims.chr.legend);
        # } else
        # {
        #   # Min dose:
        #   if( do.plot )
        ...
        if( do.plot )
        {
          l2 <- c(l2,
                  .SVG.points(x=x.origin + lmx, y=y.origin + lmy+lh,
                              pch=pch.start.event, col=bar.cols[2], cex=legend.cex*1.5,
                              class="legend-no-event", suppress.warnings=suppress.warnings),
                  .SVG.text(x=x.origin + lmx + 4*dims.chr.legend, y=y.origin + lmy+lh, text="Unmapped by software",
                            col="black", font_size=dims.chr.legend, h.align="left", v.align="center",
                            class="legend-no-event", suppress.warnings=suppress.warnings));
          # Save the info:
          .last.cma.plot.info$SVG$legend$components <<- rbind(.last.cma.plot.info$SVG$legend$components,
                                                              data.frame("string"="Unmapped by software",
                                                                         "x.start"=x.origin + lmx, "y.start"=y.origin + lmy+lh,
                                                                         "x.end"=x.origin + lmx + 3*dims.chr.legend, "y.end"=y.origin + lmy+lh,
                                                                         "x.string"=lmx + 4*dims.chr.legend, "y.string"=lmy+lh,
                                                                         "font.size"=dims.chr.legend));
        }
        lh <- lh + lnl*dims.chr.legend; lw <- max(lw, .SVG.string.dims("Unmapped by software", font_size=dims.chr.legend)["width"] + 2*dims.chr.legend);
        #lh <- lh + lnp*dims.chr.legend.title; # new para
        ...
        {
          # The legend position:
          if( is.null(x) || length(x) > 1 || is.na(x) || !(x %in% c("left", "center", "right", "farright") || is.numeric(x)) ) x <- "right";
          if( is.na(x) || x == "right" )
          {
          ...
          {
            x <- (dims.plot.x + dims.plot.width + lmx);
          } else {
  ...
  if( is.na(duration) )
  {
    ...
    #because we are not plotting length of events the above can be changed to
    latest.date <- max(cma$data$.DATE.as.Date, cmas$.FU.END.DATE, cmas$.OBS.END.DATE, na.rm=TRUE);
  ...
  if( .do.SVG ) # SVG:
  {
    ...
    # Total size needed:
    dims.total.width      <- max(dims.plot.x + dims.plot.width,  dims.legend.width);#(dims.plot.x + max(dims.plot.width,  dims.legend.width));
    ...
    # Reusable bits:
    dce1 <- .SVG.number(dims.chr.event); dce2 <- .SVG.number(dims.chr.event/2); ndce2 <- .SVG.number(-dims.chr.event/2); dce3 <- .SVG.number(dims.chr.event/3); dce4 <- .SVG.number(dims.chr.event/4); dce5 <- .SVG.number(dims.chr.event/4.5); # cache the various relative sizes used to draw the pch symbols
    svg.str[[length(svg.str)+1]] <- list(
        ...
        # pch 21:
        '<g id="pch21" stroke-width="1"> <circle cx="0" cy="0" r="',dce5,'"></circle> </g>\n',
        # pch 26 ( < ):
  ...
  if( .do.SVG ) # SVG:
  {
    ...
    .last.cma.plot.info$SVG <- list(
      ...
      "xlab"=xlab, "ylab"=ylab,
      "title"=title,
      "col.cats"=col.cats, "bar.cols"=bar.cols, "unspecified.category.label"=unspecified.category.label,
  ...
  y.vector <- c()
  x.vector <- c()
  med.class.svg.vector <- c()
  js.tooltip.svg.vector <- c()
  for( i in 1:nrow(cma$data) )
  {
    # The current plot ID:
    ...
    if( i == 1 || (cur_plot_id != cma$data[i-1,col.plotid]) )
    {
      ...
      if( highlight.observation.window )
      {
        ...
        {
          # Save the info:
          #cat(paste0("cmas$.OBS.START.DATE[s.cmas[1]]: ", cmas$.OBS.START.DATE[s.cmas[1]], "\n"))
          ...
          #cat(paste0(".last.cma.plot.info$SVG$cma$data[s.events[1],.X.OW.START] in highlight...: ", .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"], "\n"))
          #cat(paste0(".last.cma.plot.info$SVG$cma$data[s.events[1],.X.OW.END] in highlight...: ", .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"], "\n"))
          # Draw:
          svg.str[[length(svg.str)+1]] <-
            # OW:
    ...
    if( .do.SVG ) # SVG:
    {
      ...
      if (!is.null(.last.cma.plot.info$SVG$cma$event.info.cma$gap.days)) {
        gap.days.temp <- .last.cma.plot.info$SVG$cma$event.info.cma[i, "gap.days"]
      } else if (!is.null(.last.cma.plot.info$SVG$cma$event.info$gap.days)) {
        gap.days.temp <- .last.cma.plot.info$SVG$cma$event.info[i, "gap.days"]
      }

      if ( !highlight.observation.window && (i == 1 || (cur_plot_id != cma$data[i-1,col.plotid])) ) {
        .last.cma.plot.info$SVG$cma$data[s.events,".X.OW.START"] <- .scale.x.to.SVG.plot(adh.plot.space[2] + as.numeric(cmas$.OBS.START.DATE[s.cmas[1]] - earliest.date) + correct.earliest.followup.window);
        .last.cma.plot.info$SVG$cma$data[s.events,".X.OW.END"]   <- .scale.x.to.SVG.plot(adh.plot.space[2] + as.numeric(cmas$.OBS.END.DATE[s.cmas[1]]   - earliest.date) + correct.earliest.followup.window);
        #cat(paste0(".last.cma.plot.info$SVG$cma$data[s.events[1],.X.OW.START] not in highlight...: ", .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"], "\n"))
        #cat(paste0(".last.cma.plot.info$SVG$cma$data[s.events[1],.X.OW.END] not in highlight...: ", .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"], "\n"))
      }
      #.last.cma.plot.info$SVG$cma$data[s.events,".X.OW.END"]   <- .scale.x.to.SVG.plot(adh.plot.space[2] + as.numeric(cmas$.OBS.END.DATE[s.cmas[1]]   - earliest.date) + correct.earliest.followup.window);
      #.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"]
      #red.after.obs.start <- .scale.x.to.SVG.plot(adh.plot.space[2] + start + correct.earliest.followup.window - gap.days.minusone.temp)
      ...
      if ( .last.cma.plot.info$SVG$cma$data[i,".X.START"] >= .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"]) {
        svg.str[[length(svg.str)+1]] <-
          .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=min(.last.cma.plot.info$SVG$cma$data[i,".X.END"],.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"]), yend=.scale.y.to.SVG.plot(y.cur-0.5),
                    stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                    fill=bar.cols[3],
                    class = "Covered 1",
          );
      } else if ( .last.cma.plot.info$SVG$cma$data[i,".X.START"] < .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] && .last.cma.plot.info$SVG$cma$data[i,".X.END"] > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] ) {
        svg.str[[length(svg.str)+1]] <-
          .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=min(.last.cma.plot.info$SVG$cma$data[i,".X.END"],.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"]), yend=.scale.y.to.SVG.plot(y.cur-0.5),
                    stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                    fill=bar.cols[3],
                    class = "Covered 8",
          );
      ...
      if (i == nrow(.last.cma.plot.info$SVG$cma$data) || .last.cma.plot.info$SVG$cma$data[i,col.patid] != .last.cma.plot.info$SVG$cma$data[i+1,col.patid] ) {
        if ( gap.days.temp > 0 ) {
          gap.to.end.of.ow.temp <- .scale.x.to.SVG.plot(adh.plot.space[2] + as.numeric(cmas$.OBS.END.DATE[s.cmas[1]]   - earliest.date) + correct.earliest.followup.window - gap.days.temp);
          svg.str[[length(svg.str)+1]] <-
            .SVG.rect(x=max(gap.to.end.of.ow.temp,.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"]), y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                      stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ",round(min(as.numeric(cmas$.OBS.END.DATE[s.cmas[1]]-cmas$.OBS.START.DATE[s.cmas[1]]), gap.days.temp),2)),
                      fill=bar.cols[4],
                      class = "Gap 1",
            );
          if (.last.cma.plot.info$SVG$cma$data[i,".X.END"] >= .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"]) {
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.END"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=gap.to.end.of.ow.temp, yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                        fill=bar.cols[3],
                        class = "Covered 2",
              );
          } else if (.last.cma.plot.info$SVG$cma$data[i,".X.END"] < .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] && gap.to.end.of.ow.temp > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"]) {
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=gap.to.end.of.ow.temp, yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                        fill=bar.cols[3],
                        class = "Covered 6",
              )
        ...
        } else if ( gap.days.temp < 1 && .last.cma.plot.info$SVG$cma$data[i,".X.END"] < .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"]) {
          svg.str[[length(svg.str)+1]] <-
            .SVG.rect(x=max(.last.cma.plot.info$SVG$cma$data[i,".X.END"],.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"]), y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.END"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                      stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                      fill=bar.cols[3],
                      class = "Covered 7",
            )
      ...
      if ( (i==1 || .last.cma.plot.info$SVG$cma$data[i,col.patid] != .last.cma.plot.info$SVG$cma$data[i-1,col.patid]) && .last.cma.plot.info$SVG$cma$data[i,".X.START"] > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] ) {
        svg.str[[length(svg.str)+1]] <-
          .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.START"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                    stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ",round(as.numeric(cma$data$.DATE.as.Date[i]-cmas$.OBS.START.DATE[s.cmas[1]]),2)),
                    fill=bar.cols[4],
                    class = "Gap 7",
          );
      }
      ...
      if( i < nrow(cma$data) && (cur_plot_id == cma$data[i+1,col.plotid]) )
      {
          ...
          .last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"]   <- .scale.x.to.SVG.plot(adh.plot.space[2] + start.next - gap.days.temp + correct.earliest.followup.window);
          ...
          .last.cma.plot.info$SVG$cma$data[i,".X.START"] <- .scale.x.to.SVG.plot(adh.plot.space[2] + start + correct.earliest.followup.window);
          #.last.cma.plot.info$SVG$cma$data[i,".X.END"]
          if (!is.null(.last.cma.plot.info$SVG$cma$event.info.cma$gap.days) && i > 1 && (cur_plot_id == cma$data[i-1,col.plotid])) {
            gap.days.minusone.temp <- .last.cma.plot.info$SVG$cma$event.info.cma[i-1, "gap.days"]
          } else if (!is.null(.last.cma.plot.info$SVG$cma$event.info$gap.days) && i > 1 && (cur_plot_id == cma$data[i-1,col.plotid])) {
            gap.days.minusone.temp <- .last.cma.plot.info$SVG$cma$event.info[i-1, "gap.days"]
          } else {
            gap.days.minusone.temp <- 0
          }
          if (i > 1 && .last.cma.plot.info$SVG$cma$data[i,col.patid] == .last.cma.plot.info$SVG$cma$data[i-1,col.patid]) red.after.obs.start <- .scale.x.to.SVG.plot(adh.plot.space[2] + start + correct.earliest.followup.window - gap.days.minusone.temp);
          #cat(paste0(".last.cma.plot.info$SVG$cma$data[i+1,col.patid]: ", .last.cma.plot.info$SVG$cma$data[i+1,col.patid], "\n"))
          # if (is.null(.last.cma.plot.info$SVG$cma$data[i+1,col.patid]) || .last.cma.plot.info$SVG$cma$data[i,col.patid] != .last.cma.plot.info$SVG$cma$data[i+1,col.patid] ) {
          #   cat("null or new pat\n")
          # };
          ...
          if ((i == 1 || .last.cma.plot.info$SVG$cma$data[i,col.patid] != .last.cma.plot.info$SVG$cma$data[i-1,col.patid]) && .last.cma.plot.info$SVG$cma$data[i,".X.START"] > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"])
          {
            #cat("condition met1")
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.START"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ", round(as.numeric(cma$data$.DATE.as.Date[i]-cmas$.OBS.START.DATE[s.cmas[1]]),2)),
                        fill=bar.cols[4],
                        class = "Gap 2",
              );
          } else if ( i>1 && .last.cma.plot.info$SVG$cma$data[i,col.patid] == .last.cma.plot.info$SVG$cma$data[i-1,col.patid] &&
                      .last.cma.plot.info$SVG$cma$data[i,".X.START"] > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] &&
                      .last.cma.plot.info$SVG$cma$data[i-1,".X.START"] < .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"])
          {
            #cat(paste0("gap.days.minusone.temp ", gap.days.minusone.temp, "\n"))
            #cat(paste0("red.after.obs.start: ", red.after.obs.start, "\n"))
            #cat(paste0(".last.cma.plot.info$SVG$cma$data[i,.X.OW.START]: ", .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], "\n"))
            if ( gap.days.minusone.temp > 0 && red.after.obs.start >= .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"])
            {
              svg.str[[length(svg.str)+1]] <-
                .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=red.after.obs.start, yend=.scale.y.to.SVG.plot(y.cur-0.5),
                          stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                          fill=bar.cols[3],
                          class = "Covered 3",
                );
              ...
              svg.str[[length(svg.str)+1]] <-
                .SVG.rect(x=red.after.obs.start, y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.START"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                          stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ",round(gap.days.minusone.temp,2)),
                          fill=bar.cols[4],
                          class = "Gap 3",
                );
            } else if ( gap.days.minusone.temp > 0 && red.after.obs.start < .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"] ) {
              svg.str[[length(svg.str)+1]] <-
                .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.START"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                          stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ",round(as.numeric(cma$data$.DATE.as.Date[i]-cmas$.OBS.START.DATE[s.cmas[1]]),2)),
                          fill=bar.cols[4],
          ...
          if ( .last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"] >= .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"] && .last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"] <= .last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"]) {
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges",
                        fill=bar.cols[3],
                        class = "Covered 5",
              );
            ...
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.END"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ", round(gap.days.temp,2)),
                        fill=bar.cols[4],
                        class = "Gap 5",
              );
          } else if (.last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"] <= .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"] && .last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"] <= .last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"] && .last.cma.plot.info$SVG$cma$data[i,".X.CNT.END"] >= .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"]) {
            svg.str[[length(svg.str)+1]] <-
              .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.END"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
                        stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA, shape_rendering = "crispedges", js_tooltip = paste0("Gap days: ", round(as.numeric(cma$data$.DATE.as.Date[i+1]-cmas$.OBS.START.DATE[s.cmas[1]]),2)),
                        fill=bar.cols[4],
                        class = "Gap 6",
              );
          }
      ...
      if ( i > 6 && cur_plot_id == cma$data[i-6,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-6,".X.START"]) {
        y.vector <- c(y.vector, y.cur + 1.05)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
      } else if ( i > 5 && cur_plot_id == cma$data[i-5,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-5,".X.START"]) {
        y.vector <- c(y.vector, y.cur - 1.05)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
      } else if ( i > 4 && cur_plot_id == cma$data[i-4,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-4,".X.START"]) {
        y.vector <- c(y.vector, y.cur + 0.7)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
      } else if ( i > 3 && cur_plot_id == cma$data[i-3,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-3,".X.START"]) {
        y.vector <- c(y.vector, y.cur - 0.7)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
      } else if ( i > 2 && cur_plot_id == cma$data[i-2,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-2,".X.START"] ) {
        y.vector <- c(y.vector, y.cur + 0.35)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
        # svg.str[[length(svg.str)+1]] <-
        #   # The beginning of the event:
        #   .SVG.points(x=.last.cma.plot.info$SVG$cma$data[i,".X.START"], y=.scale.y.to.SVG.plot(y.cur + 0.5),
        #               pch=pch.start.event, col="black", cex=cex, #used to be col=col
        #               class=paste0("event-start",if(!is.na(med.class.svg)) paste0("-",med.class.svg)),
        #               js_tooltip=js.tooltip.svg, suppress.warnings=suppress.warnings);
      } else if ( i > 1 && cur_plot_id == cma$data[i-1,col.plotid] && .last.cma.plot.info$SVG$cma$data[i,".X.START"] == .last.cma.plot.info$SVG$cma$data[i-1,".X.START"]) {
        y.vector <- c(y.vector, y.cur - 0.35)
        x.vector <- c(x.vector, .last.cma.plot.info$SVG$cma$data[i,".X.START"])
        med.class.svg.vector <- c(med.class.svg.vector, med.class.svg)
        js.tooltip.svg.vector <- c(js.tooltip.svg.vector, js.tooltip.svg)
    ...
    if( i < nrow(cma$data) && (cur_plot_id == cma$data[i+1,col.plotid]) )
    {
      ...
      #   .last.cma.plot.info$SVG$cma$data[i,".X.START"] <- .scale.x.to.SVG.plot(adh.plot.space[2] + start + correct.earliest.followup.window);
      #   #.last.cma.plot.info$SVG$cma$data[i,".X.END"]
      #   if (!is.null(.last.cma.plot.info$SVG$cma$event.info.cma$gap.days) && i > 1) {
      #     gap.days.minusone.temp <- .last.cma.plot.info$SVG$cma$event.info.cma[i-1, "gap.days"]
      #   } else if (!is.null(.last.cma.plot.info$SVG$cma$event.info$gap.days) && i > 1) {
      #     gap.days.minusone.temp <- .last.cma.plot.info$SVG$cma$event.info[i-1, "gap.days"]
      #   }
      #   if (i > 1 && .last.cma.plot.info$SVG$cma$data[i,col.patid] == .last.cma.plot.info$SVG$cma$data[i-1,col.patid]) red.after.obs.start <- .scale.x.to.SVG.plot(adh.plot.space[2] + start + correct.earliest.followup.window - gap.days.minusone.temp);
      #   cat(paste0(".last.cma.plot.info$SVG$cma$data[i+1,col.patid]: ", .last.cma.plot.info$SVG$cma$data[i+1,col.patid], "\n"))
      ...
      #     svg.str[[length(svg.str)+1]] <-
      #       .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.OW.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.START"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
      #                 stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA,
      #                 fill="red",
      #                 class = "red at start of OW",
      #       );
      #   } else if ( i>1 && .last.cma.plot.info$SVG$cma$data[i,col.patid] == .last.cma.plot.info$SVG$cma$data[i-1,col.patid] &&
      #               .last.cma.plot.info$SVG$cma$data[i,".X.START"] > .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"] &&
      #               .last.cma.plot.info$SVG$cma$data[i-1,".X.START"] < .last.cma.plot.info$SVG$cma$data[s.events[1],".X.OW.START"])
      ...
      #   }
      #
      #   if ( .last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"] >= .last.cma.plot.info$SVG$cma$data[i,".X.OW.START"]) {
      #     svg.str[[length(svg.str)+1]] <-
      #       .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.MID"], yend=.scale.y.to.SVG.plot(y.cur-0.5),
      #                 stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA,
      #                 fill="green",
      ...
      #
      #   # svg.str[[length(svg.str)+1]] <-
      #   # .SVG.rect(x=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.START"], y=.scale.y.to.SVG.plot(y.cur+0.5), xend=.last.cma.plot.info$SVG$cma$data[i,".X.CNT.END"], yend=.scale.y.to.SVG.plot(y.cur-0.5),  # can accomodate both (wdith,height) and (xend,yend)
      #   #           stroke=NA, stroke_width=NA, lty=NA, stroke_dasharray=NA,
      #   #           fill=if(.last.cma.plot.info$SVG$cma$computed.CMA %in% c("CMA7", "CMA10", "CMA11") && is.cma.TS.or.SW && modify.continuation.line)
      #   #                   {
      #   #                     if(!is.na(.last.cma.plot.info$SVG$cma$event.info.cma[i,"gap.days"]) && .last.cma.plot.info$SVG$cma$event.info.cma[i,"gap.days"]>0) "red" else "green";
      ...
      #   #                   {
      #   #                     if(!is.na(.last.cma.plot.info$SVG$cma$event.info[i,"gap.days"]>0) && .last.cma.plot.info$SVG$cma$event.info[i,"gap.days"]>0) "red" else "green";
      #   #                   },
      #   #           class = "checkifexists",
      #   #           #fill_opacity=NA,
      #   #           # id=NA,
      ...
      #   #           # comment=NA,
      #   #           # tooltip=NA,
      #   #           # js_tooltip=NA, # ID, comment and tooltip
      #   #           # newline=TRUE, # should a newline be added at the end?
      #   #           # return_string=FALSE # return a singe string or a vector of strings to be concatenated later?
      #   # )
      # };
  ...
  for( i in 1:nrow(cma$data) )
  {
    svg.str[[length(svg.str)+1]] <-
      # The beginning of the event:
      .SVG.points(x=x.vector[i], y=.scale.y.to.SVG.plot(y.vector[i]),
                  pch=pch.start.event,
                  col=if (cma$data[i,cma$event.duration.colname] < 0.0001) bar.cols[2] else bar.cols[1], #was previously col=col # 0.0001 is used as close to zero, zero in data usually represented with a few more zeros as NA not permitted (length = NA)
                  cex=cex, class=paste0("event-start",if(!is.na(med.class.svg.vector[i])) paste0("-",med.class.svg.vector[i])),
                  js_tooltip=js.tooltip.svg.vector[i], suppress.warnings=suppress.warnings);
  }
  ##
  ...
  if( .do.SVG ) # SVG:
  {
    svg.str[[length(svg.str)+1]] <- list(
      ...
      # The x axis label:
      .SVG.text(x=(dims.plot.x + (dims.plot.width/2)), y=(dims.total.height - dims.chr.axis), #x was previously (dims.plot.x + dims.total.width)/2
    ...
    if( !is.null(export.formats) )
    {
      ...
      if( "html" %in% export.formats )
      {
        ...
        if( is.null(export.formats.html.css) )
        {
          ...
          css.template.path <- system.file('html-templates/css-template.css', package='AdhereRSTUMedView');
          if( is.null(css.template.path) || css.template.path=="" )
          {
            ...
            assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adhererstumedview.env); # save the plot infor into the environment
            plot.CMA.error(export.formats=export.formats,
                           export.formats.fileprefix=export.formats.fileprefix,
                           export.formats.directory=export.formats.directory,
        ...
        if( is.null(export.formats.html.javascript) )
        {
          ...
          js.template.path <- system.file('html-templates/javascript-template.js', package='AdhereRSTUMedView');
        ...
        if( is.null(export.formats.html.template) && html.plot.number == 1) #load in the base html template if first/only plot in the block
        {
          ...
          html.template.path <- system.file('html-templates/html-template.html', package='AdhereRSTUMedView');
          if( is.null(html.template.path) || html.template.path=="" )
          {
            ...
            .last.cma.plot.info$SVG <- NULL;
            assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adhererstumedview.env); # save the plot info into the environment
            plot.CMA.error(export.formats=export.formats,
                           export.formats.fileprefix=export.formats.fileprefix,
                           export.formats.directory=export.formats.directory,
                           generate.R.plot=FALSE);
  ...
  assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adhererstumedview.env);
...
)
{
  ...
  # No last plot (really)...
  assign(".last.cma.plot.info", NULL, envir=.adhererstumedview.env);
