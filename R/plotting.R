  ...
  for( i in 1:nrow(cma$data) )
  {
    svg.str[[length(svg.str)+1]] <-
      ...
      .SVG.points(x=x.vector[i], y=.scale.y.to.SVG.plot(y.vector[i]),
                  pch=pch.start.event, col=if (cma$data[i,cma$event.duration.colname] < 0.0001) bar.cols[2] else bar.cols[1], #was previously col=col # 0.0001 is used as close to zero, zero in data usually represented with a few more zeros as NA not permitted (length = NA)
