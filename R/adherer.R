...
assign(".ewms", NULL, envir=.adhererstumedview.env);
assign(".record.ewms", FALSE, envir=.adhererstumedview.env); # initially, do not record the errors, warnings and message, but just display them

# Clear the info about errors, warnings and messages:
.clear.ewms <- function() { assign(".ewms", NULL, envir=.adhererstumedview.env); }
...
.get.ewms <- function() { return (get(".ewms", envir=.adhererstumedview.env)); }

# Are we recording the errors, warnings and messages?
.is.recording.ewms <- function() { return (!is.null(.record.ewms <- get(".record.ewms", envir=.adhererstumedview.env)) && .record.ewms); }

# Start/stop recording the errors, warnings and messages:
.record.ewms <- function(record=TRUE) { .clear.ewms(); assign(".record.ewms", record, envir=.adhererstumedview.env); }
...
#' @param bar.cols A vector of 4 colours to be used for bar style plots; 1=event, 2=zero event,
#' 3=covered & 4=notcovered.
...
#' @export
plot_summary.CMA0 <- function(x,                                     # the CMA0 (or derived) object
                      ...,                                   # required for S3 consistency
                      ...
                      bar.cols = c("#403e3e", "#0011ff", "#4ebf5c", "#FDB623"), # vector of 4 colours used for bar plots
...
)
{
    ...
    if( inherits(time.interval, "difftime") ) # check if a difftime and, if so, attempt conversion to number of units
    {
      ...
      } else
      {
        ...
        time.interval <- switch( as.character(unit),
                                 ...
                                 "months" = {if( !suppress.warnings ) .report.ewms("Converting to months assuming a 30-days month!", "warning", ".add.time.interval.to.date", "AdhereRSTUMedView"); time.interval/30;},
                                 "years"  = {if( !suppress.warnings ) .report.ewms("Converting to years assuming a 365-days year!", "warning", ".add.time.interval.to.date", "AdhereRSTUMedView"); time.interval/365;},
                                 {if( !suppress.warnings ) .report.ewms(paste0("Unknown unit '",unit,"' to '.add.time.interval.to.date'.\n"), "error", ".add.time.interval.to.date", "AdhereRSTUMedView"); return(NA);} # default
        );
      }
    } else
    {
  ...
  return (switch( as.character(unit),
                            ...
                            {if( !suppress.warnings ) .report.ewms(paste0("Unknown unit '",unit,"' to '.add.time.interval.to.date'.\n"), "error", ".add.time.interval.to.date", "AdhereRSTUMedView"); NA;} # default
  ));
}
...
)
{
  ...
  maximum.permissible.gap <- switch(maximum.permissible.gap.unit,
                                    ...
                                    {if(!suppress.warnings) .report.ewms(paste0("Unknown maximum.permissible.gap.unit '",maximum.permissible.gap.unit,"': assuming you meant 'days'."), "warning", "compute.treatment.episodes", "AdhereRSTUMedView");  maximum.permissible.gap;} # otherwise force it to "days"
                                   );
...
#
############################################################################################
.plot_summary.CMA1plus <- function(cma,                                   # the CMA1 (or derived) object
                           patients.to.plot=NULL,                 # list of patient IDs to plot or NULL for all
                           duration=NA,                           # duration to plot in days (if missing, determined from the data)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA1", "AdhereRSTUMedView");
      }
    }
  }
...
#' @export
plot_summary.CMA1 <- function(x,                                     # the CMA1 (or derived) object
                      ...,                                   # required for S3 consistency
                      patients.to.plot=NULL,                 # list of patient IDs to plot or NULL for all
                      duration=NA,                           # duration to plot in days (if missing, determined from the data)
                      align.all.patients=FALSE, align.first.event.at.zero=TRUE, # should all patients be aligned? and, if so, place the first event as the horizintal 0?
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA2", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA2 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA3", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA3 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA4", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA4 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA5", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA5 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA6", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA6 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA7", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA7 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA8", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA8 <- function(...) .plot_summary.CMA1plus(...)
                ...
                )
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA9", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA9 <- function(...) .plot_summary.CMA1plus(...)
...
)
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA10", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA10 <- function(...) .plot_summary.CMA1plus(...)
...
)
{
  ...
  if( !suppress.warnings && !is.null(arguments.that.should.not.be.defined) )
  {
    ...
    if( any(args.mathing) )
    {
      for( i in which(args.mathing) )
      {
        .report.ewms(paste0("Please note that '",args.list[[1]],"' overrides argument '",names(arguments.that.should.not.be.defined)[i],"' with value '",arguments.that.should.not.be.defined[i],"'!\n"), "warning", "CMA11", "AdhereRSTUMedView");
      }
    }
  }
...
#' @rdname plot_summary.CMA1
#' @export
plot_summary.CMA11 <- function(...) .plot_summary.CMA1plus(...)
...
subsetCMA.CMA_per_episode <- function(cma, patients, suppress.warnings=FALSE)
{
  ...
  if( length(patients.to.keep) == 0 )
  {
    if( !suppress.warnings ) .report.ewms("No patients to subset on!\n", "error", "subsetCMA.CMA_per_episode", "AdhereRSTUMedView");
    return (NULL);
  }
  if( length(patients.to.keep) < length(patients) && !suppress.warnings ) .report.ewms("Some patients in the subsetting set are not in the CMA itself and are ignored!\n", "warning", "subsetCMA.CMA_per_episode", "AdhereRSTUMedView");
...
)
{
  ...
  {
    .report.ewms("Unknown format for printing!\n", "error", "print.CMA_per_episode", "AdhereRSTUMedView");
    return (invisible(NULL));
  }
}
...
#' @export
plot_summary.CMA_per_episode <- function(x,                                     # the CMA_per_episode or CMA_sliding_window (or derived) object
                                 patients.to.plot=NULL,                 # list of patient IDs to plot or NULL for all
...
)
{
  ...
  {
    if (!suppress.warnings ) .report.ewms("CMA10/11 is not suitable for more than 1 sliding window as output will be misleading. If you think this error is a mistake please try using the same UOM for sliding.window.duration and sliding.window.step.duration", "error", "CMA_sliding_window", "AdhereRSTUMedView")
    return (NULL)
  }
  ...
  {
    if (!suppress.warnings ) .report.ewms("First event OW window if only for use on a single step window on CMA10/CMA11 starting at start of OW (STUMedView tweak as oppose to full feature).", "error", "CMA_sliding_window", "AdhereRSTUMedView")
    return (NULL)
  }
  if( is.numeric(sliding.window.start) && sliding.window.start < 0 )
  {
    if( !suppress.warnings ) .report.ewms("The sliding window must start a positive number of time units after the start of the observation window!\n", "error", "CMA_sliding_window", "AdhereRSTUMedView")
    return (NULL);
  }
  ...
  CMA.FNC <- switch(as.character(CMA.to.apply), # if factor, force it to string
                    ...
                    "CMA10" = CMA10,
                    "CMA11" = CMA11,
                    {if( !suppress.warnings ) .report.ewms(paste0("Unknown 'CMA.to.apply' '",CMA.to.apply,"': defaulting to CMA0!\n)"), "warning", "CMA_sliding_window", "AdhereRSTUMedView"); CMA0;}); # by default, fall back to CMA0
  ...
  {
    carryover.into.obs.window <- carryover.within.obs.window <- FALSE;
    if( !is.na(carry.only.for.same.medication) && carry.only.for.same.medication && !suppress.warnings ) .report.ewms("'carry.only.for.same.medication' cannot be defined for CMAs 1-4!\n", "warning", "CMA_sliding_window", "AdhereRSTUMedView");
    carry.only.for.same.medication <- FALSE;
    if( !is.na(consider.dosage.change) && consider.dosage.change && !suppress.warnings ) .report.ewms("'consider.dosage.change' cannot be defined for CMAs 1-4!\n", "warning", "CMA_sliding_window", "AdhereRSTUMedView");
    consider.dosage.change <- FALSE;
  } else if( CMA.to.apply %in% c("CMA5", "CMA6") )
...
#' @rdname plot_summary.CMA_per_episode
#' @export
plot_summary.CMA_sliding_window <- plot_summary.CMA_per_episode;
